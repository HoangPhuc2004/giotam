  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 pb-12 relative w-full overflow-hidden">
      ...
      <div className="px-6 mt-8 space-y-8 max-w-7xl mx-auto">
        ...
        {userRole === 'hospital' && (
          <div className="bg-[#FBF2E1] rounded-3xl p-6 md:p-8 shadow-lg border border-[#e5d5b7]">
            <h2 className="text-xl font-bold text-gray-900 mb-2">Tìm người hiến máu</h2>
            <p className="text-sm text-gray-600 mb-6">Gửi thông báo khẩn cấp tới những tình nguyện viên phù hợp xung quanh theo bán kính.</p>
            
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <div className="flex-1">
                <select 
                  className="w-full h-12 bg-white border border-gray-300 rounded-xl px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#930511]"
            ...
            <button 
              ...
              className="w-full sm:w-auto bg-[#930511] hover:bg-[#7a0410] text-white h-12 px-8 rounded-xl text-sm font-bold flex items-center justify-center disabled:opacity-70 transition-colors"
            >
              <Search className="w-5 h-5 mr-2" /> {isSearching ? 'Đang tìm...' : 'Tìm kiếm'}
            </button>

            {searchError && <p className="text-sm text-red-600 mt-4 font-medium">{searchError}</p>}

            {/* Results */}
            {searchResults.length > 0 && (
              <div className="mt-8 border-t border-[#e5d5b7] pt-6">
                <div className="flex items-center justify-between mb-4">
                  <label className="flex items-center gap-3 text-sm font-semibold text-gray-900 cursor-pointer">
                    <input type="checkbox" className="w-5 h-5 rounded accent-[#930511]" checked={selectedDonors.length === searchResults.length} onChange={handleSelectAll} />
                    <span>Chọn tất cả ({selectedDonors.length}/{searchResults.length})</span>
                  </label>
                  <button onClick={handleBulkContact} disabled={isSearching || selectedDonors.length === 0} className="bg-[#930511] hover:bg-[#7a0410] text-white text-sm px-5 py-2.5 rounded-xl font-bold disabled:opacity-50 flex items-center transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2">
                  {searchResults.map((result: any) => (
                    <label key={result.user.id} className={`flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${selectedDonors.includes(result.user.id) ? 'bg-[#fbe4e6] border-[#930511]' : 'bg-white border-transparent hover:border-gray-200 shadow-sm'}`}>
                      <input type="checkbox" className="w-5 h-5 rounded accent-[#930511] shrink-0" checked={selectedDonors.includes(result.user.id)} onChange={() => handleSelectDonor(result.user.id)} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                          <h4 className="font-bold text-base text-gray-900 truncate">{result.user.name}</h4>
                          <span className="bg-[#930511]/10 text-[#930511] text-xs font-bold px-2 py-1 rounded inline-flex items-center shrink-0">
                            <Droplet className="w-3 h-3 mr-1" /> {result.user.blood_type}
                          </span>
                        </div>
                        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
                          <span className="flex items-center"><Phone className="w-4 h-4 mr-1.5" /> {result.user.phone}</span>
                          <span className="flex items-center"><MapPin className="w-4 h-4 mr-1.5" /> {result.distance_km.toFixed(1)} km</span>
                        </div>
                      </div>
                    </label>
                  ))}
